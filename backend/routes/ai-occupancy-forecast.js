// AI Occupancy forecasting & promotions
// Predict occupancy, auto-generate promotional pricing to fill gaps
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const response = await fetch(baseUrl + '/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIRVParkCampgroundManager'
    },
    body: JSON.stringify({
      model: MODEL,
      messages: [
...
router.post('/', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert AI assistant for AIRVParkCampgroundManager. Focus area: Occupancy forecasting & promotions. ${`Predict occupancy, auto-generate promotional pricing to fill gaps`}. Respond ONLY with valid JSON (no markdown fences).`;
    const userPrompt = `Task: Occupancy forecasting & promotions.\n${`Predict occupancy, auto-generate promotional pricing to fill gaps`}\n\nInput payload (JSON):\n${JSON.stringify(context, null, 2)}\n\nReturn JSON with the shape:\n{\n  "summary": "...",\n  "findings": ["..."],\n  "recommendations": ["..."],\n  "score": 0,\n  "confidence": 0\n}`;
    ...
    await persistResult(req.user?.id, 'occupancy-forecast', context, parsed);
    res.json({ feature: 'occupancy-forecast', model: MODEL, result: parsed });
  } catch (err) {
    console.error('[occupancy-forecast]', err.message);
...
router.get('/history', async (req, res) => {
  try {
    const r = await pool.query('SELECT id, endpoint, input_data, result, created_at FROM ai_results WHERE endpoint=$1 ORDER BY created_at DESC LIMIT 50', ['occupancy-forecast']);
